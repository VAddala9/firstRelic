...
@Autonomous(name="BigRobotWallFollowerTimer", group="Psionics")
//@Disabled
public class BigRobotWallFollowerTimer extends LinearOpMode {
    ...
    public double returnDistance2(double sensorValuen) {
        double sensorValue2 = (sensorValuen * 10);
        if(sensorValue2 >= .38)
        {
            dist2 = 1.0;
        }
        else if(sensorValue2 < .38 && sensorValue2 >=.132)
        {
            dist2 = 2.0;
        }

        else if(sensorValue2 < .132 && sensorValue2 >=.058)
        {
            dist2 = 3.0;
        }
        else if(sensorValue2 < .058 && sensorValue2 >=.039)
        {
            dist2 = 4.0;
        }
        ...
        else if(sensorValue2 < .029 && sensorValue2 >=.024)
        {
            dist2 = 6.0;
        }
        else if(sensorValue2 < .024 && sensorValue2 >=.02)
        {
            dist2 = 8.0;
        }
        else
    ...
    public void followWall(double sensorValue, double sensorValue2) throws InterruptedException {
        ...
        while(1==1)
        {
            ...
            if (dist < 4.0) {
                ...
                timedDrive(.5, -.5, .5, -.5, .75);
                timedDrive(.5, .5, .5, .5, 1);
                timedDrive(-.5, .5, -.5, .5, .75);
                // sleep(500);
                sensorValue = ODsensorRF;
                sensorValue2 = ODsensorRB;
